...
theorem conservative_true : `[qsl Var| qTrue] s = 1 ↔ `[sl Var| sTrue] s := by
  apply Iff.intro
  · simp only [slTrue, implies_true]
  · simp only [qslTrue, implies_true]

theorem conservative_false : `[qsl Var| qFalse] s = 1 ↔ `[sl Var| sFalse] s := by
  apply Iff.intro
  · simp only [qslFalse, zero_ne_one, false_implies]
  · simp only [slFalse, Bool.false_eq_true, false_implies]
